...
program define brain, rclass
	...
	if `"`1'"' == substr("train",1,length("`1'")) {
		...
		forvalue i = 1/`iter' {
			...
			// 0.01 ≤ η ≤ 0.9, dropping with iterations. Start large, end small.
			
			if inlist(`i',int(`iter'*1/4),int(`iter'*2/4),int(`iter'*3/4)){ 
				
				local eta=`eta'/2 								
				
			}
			
			/* Custom code ends here  (other than adding eta to output)*/
			
			mata: braintrain(`eta', `N')
			local err = r(error)/`N'/colsof(output)
			local delta = `err'-`prev'
			local prev = `err'
			di as result %9.0f `i' as text " {c |} " as result %12.9f `err' as text " {c |} " as result %12.9f `delta' as text " eta = `eta'"
		}
		mata: braintrain(0, `N')
		local err = r(error)/`N'/colsof(output)
		local delta = `err'-`prev'
